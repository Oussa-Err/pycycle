...
class FormGestionFormateur:
    def __init__(self):
          ...
          self.lblmatricule=tk.Label(self.frameLableEntry,text="Matricule")
          self.lblmatricule.grid(row=3,column=0,pady=10)
          self.entrymatricule=tk.Entry(self.frameLableEntry)
          self.entrymatricule.grid(row=3,column=1)
    ...
    def add(self):
         try:
               self.etab1.ajouter(Formateur(self.entrymatricule.get(),self.entryLogin.get(),self.entryPWD.get()))
               self.clear()
               self.entryLogin()
